...
<?php
    ...
    // $weekly = mysqli_query($connection, "SELECT YEAR(start_date) AS year, WEEK(start_date, 1) AS week, SUM(rb_issue) AS total_rb_issue, SUM(lb_issue) AS total_lb_issue
    // FROM water_issue
    // WHERE start_date <= NOW()
    // GROUP BY year, week");

    $weekly = mysqli_query($connection, "SELECT 
    YEAR(start_date) AS year, 
    CONCAT(DATE_FORMAT(start_date, '%M'), ' ',
        CASE 
            WHEN ((DAY(start_date) - 1) DIV 7) + 1 = 1 THEN '1st'
            WHEN ((DAY(start_date) - 1) DIV 7) + 1 = 2 THEN '2nd'
            WHEN ((DAY(start_date) - 1) DIV 7) + 1 = 3 THEN '3rd'
            ELSE CONCAT(((DAY(start_date) - 1) DIV 7) + 1, 'th')
        END,
        ' week'
    ) AS month_and_week,
    ...
    FROM water_issue
    WHERE start_date <= NOW()
    GROUP BY year, month_and_week
    ORDER BY start_date ASC");



    if(mysqli_num_rows($weekly)>0){   
        $html='<h2 style="text-align:center";>Weekly Detailed Report</h2>';  // Heading
        ...
        $html.='<table style="border-collapse: collapse; width: 100%;">';
            ...
            <tr style="background-color: #f2f2f2;">
                ...
                <td style="padding: 8px; border: 1px solid #ddd; font-weight: bold;">Week</td>
            ...
            while($row=mysqli_fetch_assoc($weekly)){
                $html.=
                '<tr>  
                    <td style="padding: 8px; border: 1px solid #ddd;">'.$row['year'].'</td>
                    <td style="padding: 8px; border: 1px solid #ddd;">'.$row['month_and_week'].'</td>
