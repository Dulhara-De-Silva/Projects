...
<?php
    ...
    $daily = mysqli_query($connection, "SELECT start_date, SUM(rb_issue) AS total_rb_issue, SUM(lb_issue) AS total_lb_issue
    FROM water_issue
    GROUP BY start_date");

    if(mysqli_num_rows($daily)>0){   
        $html='<h2 style="text-align:center";>Daily Detailed Report</h2>';  // Heading
        ...
        // Values for the table
            while($row=mysqli_fetch_assoc($daily)){
                $html.=
                '<tr>
                    <td style="padding: 8px; border: 1px solid #ddd;">'.$row['start_date'].'</td>
