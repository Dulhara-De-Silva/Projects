...
<?php
    ...
    $yearly = mysqli_query($connection, "SELECT YEAR(start_date) AS year, SUM(rb_issue) AS total_rb_issue, SUM(lb_issue) AS total_lb_issue
    FROM water_issue
    WHERE start_date <= NOW()
    GROUP BY YEAR(start_date)");

    if(mysqli_num_rows($yearly)>0){   
        $html='<h2 style="text-align:center";>Yearly Detailed Report</h2>';  // Heading
